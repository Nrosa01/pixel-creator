...
<template>
  <div class="flex flex-wrap w-full grow gap-2 items-start bg-slate-600/50 rounded-xl p-2 content-start h-full" v-auto-animate>
    <button @mouseenter="playHover" v-for="(particle, i) in particle_array" :key="particle.data.name" class="border-2 border-black hover:bg-slate-300 hover:scale-105 font-semibold py-1 px-2 rounded" :class="selected == i ? 'border-white font-extrabold text-white stroke-slate-900 stroke-4' : ''" :style="`background-color: rgb(${particle.data.color.join(',')});`" @click="selectParticle(i)">
      <span :class="selected == i ? 'drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]' : ''">{{ particle.display_name }}</span>
    </button>
    <button @mouseenter="playHover" :class="buttonClass + ' font-black'" title="Add a new particle" @click="add">+</button>
    <button @mouseenter="playHover" :class="buttonClass + (selected == 0 ? ' brightness-50' : '') + ' font-black'" :disabled="selected == 0" title="Remove selected particle" @click="removeSelected">-</button>
  </div>
</template>
