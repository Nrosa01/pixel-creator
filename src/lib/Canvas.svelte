...
<div class="h-screen">
<div class="grid grid-cols-2 place-items-baseline h-full justify-center bg-slate-300 py-8">
  <div class="aspect-square ml-8 h-full bg-slate-600 border-2 border-black">
    <canvas class="w-full h-full cursor-none" id="glcanvas"></canvas>
  </div>

  <div class="flex flex-col pr-8 w-full h-full">
    <div class="flex justify-between items-center bg-slate-600/50 rounded-xl mb-4 p-2">
      ...
      <button title="Open particle editor" class="{button_class}" on:click={openEditor}>Open editor</button>
  </div>
